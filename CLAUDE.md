...
- Weekly incremental (`--recent-only`): ~50-150 credits/week. GitHub Actions cron Mondays 6:30am ET. **Firecrawl `maxConcurrency` is 5 — never pass `--workers` above 5**, and credits are annual (5,000/period), not monthly; see "Firecrawl Account Limits" in AGENTS.md for the live queries.
