"""State helpers for the Maine Listings scraping pipeline."""
...
_DEFAULT_STATE_PATH = os.path.join(
    os.path.dirname(__file__), '..', 'data', 'maine_scrape_state.json',
)
...
def save_state(state: dict, path: str | None = None) -> None:
    ...
    fd, tmp_path = tempfile.mkstemp(
        dir=os.path.dirname(os.path.abspath(path)), suffix='.json',
    )
    try:
...
def mark_complete(state: dict, town: str, listings_found: int = 0) -> None:
    state['towns'][_town_slug(town)] = {
        'status': 'complete',
